...
Client::Client(std::string login, const std::string &password, const KeyDistributionCenter &kdc)
    ...
    void Client::request_tgt() {
        ...
        save_tgt(authenticationResultToken.ticketGrantingTicket, decryptedToken.expiration_timestamp, decryptedToken.current_timestamp);
    ...
    void Client::request_service_ticket(const std::string &server_name) {
        ...
        ticketGrantingResultToken.clientResponseToken = ticketGrantingResultToken.clientResponseToken.decrypt(
                tgtData.sessionKey);
        std::cout << "Client decrypted TicketGrantingResultToken(ClientResponseToken + ServerToken) with the TGT session key" << std::endl;
        std::cout << "        TGS_SESSON_KEY: " << tgtData.sessionKey << std::endl;
        std::cout << "        ServerName: " << ticketGrantingResultToken.clientResponseToken.serverName << std::endl;
        std::cout << "        EXPIRATION_TIME: " << ticketGrantingResultToken.clientResponseToken.expirationTimestamp << std::endl;
        std::cout << "        CURRENT_TIME: " << ticketGrantingResultToken.clientResponseToken.currentTimestamp << std::endl;
        std::cout << "        SERVICE_SESSON_KEY: " << ticketGrantingResultToken.clientResponseToken.sessionKey << std::endl;
    ...
    void Client::verify_connection(const Server &server) {
        ClientToken clientToken(login, boost::posix_time::to_simple_string(tgtData.currentTimestamp));
    ...
    void Client::save_tgt(TicketGrantingTicket &tgt, const std::string &expirationTimestamp, const std::string& currentTimestamp) {
        tgtData.ticketGrantingTicket = tgt;
        try {
            tgtData.expirationTimestamp = boost::posix_time::time_from_string(expirationTimestamp);
            tgtData.currentTimestamp = boost::posix_time::time_from_string(currentTimestamp);
        } catch (...) {
    ...
    [[nodiscard]] ClientToken Client::generate_client_token() const {
        ...
        std::cout << "   current_timestamp: " << clientToken.current_timestamp << std::endl;
    ...
    void Client::addServerData(const TicketGrantingResultToken &tgr) {
        boost::posix_time::ptime expirationTimestamp = boost::posix_time::time_from_string(tgr.clientResponseToken.expirationTimestamp);
        boost::posix_time::ptime currentTimestamp = boost::posix_time::time_from_string(tgr.clientResponseToken.currentTimestamp);
        ServerData serverData{tgr.serviceTicket, expirationTimestamp,currentTimestamp,
